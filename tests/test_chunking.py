...
def test_long_page_produces_expected_character_windows() -> None:
    ...
    chunks = chunk_pages([PDFPage(page_number=1, text=text)])
...
def test_every_chunk_is_an_exact_source_substring() -> None:
    ...
    chunks = chunk_pages([PDFPage(page_number=4, text=text)])
...
def test_blank_page_produces_no_chunks(text: str) -> None:
    assert chunk_pages([PDFPage(page_number=1, text=text)]) == []
...
def test_large_page_number_uses_at_least_three_digits() -> None:
    chunks = chunk_pages([PDFPage(page_number=1234, text="Purchase Order")])

    assert chunks[0].chunk_id == "page-1234-chunk-001"
